-- Atomic transfer and receive-transfer RPCs for barcode stock.
-- Safe to re-run in Supabase SQL Editor.

drop function if exists public.transfer_stock_unit(text, uuid, text, text);
...
begin
  ...
  if unit_record.transfer_to_location_id <> p_receive_location_id then
    raise exception 'Barcode is not assigned to this receiving location.';
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      'barcode', unit_record.barcode,
      'fromLocationId', unit_record.location_id,
      'toLocationId', p_receive_location_id,
      'atomic', true
    )
  );
