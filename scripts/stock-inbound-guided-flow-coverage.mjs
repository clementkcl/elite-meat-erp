...
const submitContextStart = workflowForms.indexOf(
  "data-stock-action=\"guided-inbound-submit-context\""
)
const submitContextEnd = workflowForms.indexOf(
  "data-stock-action=\"guided-inbound-flow\"",
  submitContextStart
)
assert(submitContextStart >= 0, "Guided inbound submit context missing")
assert(submitContextEnd > submitContextStart, "Guided inbound submit context boundary missing")
const submitContext = workflowForms.slice(submitContextStart, submitContextEnd)
includesAll(
  submitContext,
  [
    'name="brandName"',
  ...
  "Guided inbound custom-name submit context"
)
for (const duplicateField of [
  ...
  "barcodeWeightStart",
  "barcodeWeightLength",
  "barcodeWeightDecimals",
]) {
  assert(
    !submitContext.includes(`name="${duplicateField}"`),
    `Guided inbound submit context must not duplicate visible field ${duplicateField}.`
  )
}
...
includesAll(
  ...
  [
    ...
    "canonicalLookupName(readString(row.name)) === lookupName",
