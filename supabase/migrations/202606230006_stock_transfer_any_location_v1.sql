-- Stock transfer location MVP.
-- Safe to re-run: replaces transfer-related functions without destructive table changes.
-- Allows workers to select any active, scoped stock location as the transfer destination.
...
begin
  ...
  if new.status = 'TRANSFER_PENDING'
    ...
    and not exists (
      ...
      where location.id = new.transfer_to_location_id
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'transferDestinationAnyActiveLocation', p_outbound_type = 'TRANSFER',
      'atomic', true
    )
  );
...
grant execute on function public.confirm_direct_outbound_batch(text, uuid, jsonb, text, text, text) to authenticated;
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'transferDestinationAnyActiveLocation', true,
      'atomic', true
    )
  );
