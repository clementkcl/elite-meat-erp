...
| Inventory / Stock | Strong partial. Numeric-only category+default-brand+product item master with all-role create/edit and admin/director delete, barcode scanner with success feedback and continuous mode for batch workflows, atomic continuous inbound with recent templates, product search, quick product create, auto batch number, assigned-location default, duplicate auto-save prevention, no-weight barcode blocking, current preset display, saved scan count, saved total weight, previous scan weight, recent inbound scan/label list, finish-session summary, current-session undo that voids with audit trail, generated numeric barcode labels, inbound label PDF print/export, scoped stock-unit detail/reprint page, item+brand+origin barcode rules, order-based outbound with requested-vs-scanned/substitution warning plus missing-barcode and blocked-status pre-confirm warnings, direct outbound, atomic transfer, atomic receive-transfer, atomic return, atomic customer-return inspection release, damage/spoilage request with manager review/director approval status/scope gates, return-supplier request with manager approval status/scope gates, no-barcode-to-barcode redirect/guidance, scoped barcode-only stock take with empty-session submit/review/approval blocking, missing-barcode variance calculation at director approval, manager review/director approval signatures, and manual count-line blocking, stock dashboard KPIs including today inbound/outbound, stock dashboard shortcuts for inbound/outbound/transfer/receive/return, stock balance/inbound-age/variance/damage/supplier-return reports, negative-stock alerting, stock-age alerting, overdue transfer receive alerting, legacy no-barcode visibility for old records, item-code regression checks, numeric barcode-label regression checks, outbound-rule regression checks, stockable-status regression checks, stock-take scope/signature regression checks, damage/return-supplier approval regression checks, report export regression checks, stock acceptance coverage guards, stock state-transition coverage guards, stock migration safety guards, stock role/scope coverage guards, stock scanner coverage guards, stock RLS policy coverage guards, stock seed coverage guards, stock security/no-service-role guards, stock item-master coverage guards, stock label coverage guards, stock-take lock coverage guards, and stock report coverage guards exist. Configured Supabase stock data errors now surface instead of silently falling back to demo data. Needs real RLS, real scanner, and stock edge-case QA. | `/stock/*`, `components/stock/*`, `lib/stock/*` |
...
- Real Supabase/RLS QA for manager-created, item+brand-scoped stock take sessions and director final approval.
...
- Stock take now uses item+brand scoped sessions, blocks open-count item+brand stock movement at that location, and uses an atomic director approval RPC after manager review to compute missing barcode variance, adjust missing units out, write adjustment movements, and approve the session.
