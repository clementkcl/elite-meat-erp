...
| Location defaults to worker assigned stock location and remains editable. | Source guarded | `defaultLocationId`, `assignedInboundLocationId`, and `Default location:` are covered; saved local presets no longer override an active assigned location on page open, and the location picker remains editable until the session is locked. | Confirm live scoped worker default location in Supabase. |
| One inbound session locks item + brand + origin + location. | Source guarded | Session setup lock is checked in Stock mobile source guard, and the inbound scanner shows `Scanning inbound` plus `Session locked. Finish before changing setup.` after the first saved scan. | Save first scan, then confirm setup cannot be changed until finish. |
